...
;-
function gdlffdicom_shared::Open, filename, IMPLICIT_VR=implicit_vr, EXPLICIT_VR=explicit_vr, ACCESS_TIME=access_time, BIG_ENDIAN=big_endian, LITTLE_ENDIAN=little_endian, $
LIMIT=limit, INDEX_TAGS=index_tags, META_ONLY=meta_only, AUTO_SYNTAX=auto_syntax, NO_CATCH=no_catch, RAISE=raise, INDEX_SEQUENCES=index_sequences, MAX_BYTES=max_bytes
...
;if (self.lun ne 0) then message, LEVEL=-1, "Cannot open another file while one is already open. Try gdlffdicom_shared::close"
...
if (keyword_set(auto_syntax)) then begin
    ...
    if (self -> open(filename,  /IMPLICIT_VR, /LITTLE_ENDIAN, /META_ONLY, INDEX_TAGS=index_tags, /NO_CATCH,  MAX_BYTES=max_bytes)) then begin
...
self -> openu, filename,  MAX_BYTES=max_bytes
...
;stat = fstat(lun)
if (n_elements(limit) eq 0) then limit = self -> fsize() ; stat.size
...
;-
pro gdlffdicom_shared::Dump, LUN=lun
...
;-
function gdlffdicom_shared::findtaginds, group_number, element_number, COUNT=count
...
;-
function gdlffdicom_shared::readelement, group_number, element_number, value_out, OFFSET=offset, INDEX=index, VR=vr
...
;-
function gdlffdicom_shared::generatevalue, vr, len
...
function gdlffdicom_shared::NewSOPInstanceUID, group_number, element_number
...
function gdlffdicom_shared_GenerateUID, length, EMPTY_ON_ERROR=eoe, ERROR=error
common gdlffdicom_shared, instance_id, last_time, root, seed
...
;could be generated.
;-
function gdlffdicom_shared::NewUID, group_number, element_number
...
if (self -> readelement(group_number,element_number, SOPInstanceUID, OFFSET=offset, VR='UI')) then begin
    ...
    SOPInstanceUID = gdlffdicom_shared_GenerateUID(strlen(SOPInstanceUID),/EMPTY_ON_ERROR,ERROR=error)
    if ((SOPInstanceUID ne '')) then begin
        self -> point_lun, offset
        self -> write_lun, SOPInstanceUID
...
;-
function gdlffdicom_shared::assoc, INDEX=index, IMAGE=value, COUNT=count, ACCESS_TIME=access_time, NO_CATCH=no_catch, RAISE=raise, TRUE=true
...
if ((index ge 0) and (index lt self.size)) then begin
    ...
    if (arg_present(access_time) or (n_elements(access_time) gt 0)) then access_time = systime(1) - start_time
    return, self -> map(value,(*self.offsets)[index], count)
;    return, assoc(self.lun, value, (*self.offsets)[index])
endif else count = 0

end

function gdlffdicom_shared::nbytes, value
s = size(value)
case (s[s[0l]+1l]) of
...
endcase
if (nbytes mod 2 eq 1) then message, "No single byte offsets allowed"
return, nbytes
end
;+
...
;-
pro gdlffdicom_shared::write, filename, values
...
self -> openu, filename, /WRITE

self -> writeu, bytarr(128), /WRITE
self -> writeu, 'DICM', /WRITE
...
for index=0l,self.size-1l do begin
    ...
    self -> writeu, tag[0:1], /WRITE
        
    if (~is_delimiter and (self.explicit_vr or inside_metadata)) then begin

        self -> writeu, strmid(vr,0,2), /WRITE

        vr_inds = where(vr eq ['OB', 'OW', 'OF', 'SQ', 'UT', 'UN'],vr_count)
        if (vr_count gt 0) then begin

            self -> writeu, uint('0000'x), /WRITE
    ...
    self -> writeu, write_len, /WRITE  


    if ((vr ne 'SQ') and ~is_delimiter and self.index_sequences) then begin
        if (v_count gt 0) then begin
            self -> writeu, *values[i].value, /WRITE
        endif else begin
            offset = (*self.offsets)[index]
            ;size = (fstat(self.lun)).size
            fsize = self -> fsize()
            if (long64(offset) + len le fsize) then begin
                self -> point_lun, offset
                self -> copy_lun, len
            endif else begin
                print, "Warning, truncating bytes"
                self -> point_lun, offset
                self -> copy_lun, fsize - offset
            endelse
        endelse
    endif
endfor
...
;-
function gdlffdicom_shared::init
...
pro gdlffdicom_shared::openu, handle, WRITE=write, MAX_BYTES=max_bytes
;if (n_elements(max_bytes) eq 0) then self.max_bytes = 10 * 1024l * 1024l ; max_bytes
self.max_bytes = max_bytes
if (keyword_set(write)) then begin
  self.write_handle = '/'+handle 
  self.write_offset = 0l
endif  else begin
...
pro gdlffdicom_shared::free_lun, WRITE=write
if (keyword_set(write)) then begin
    self.write_offset = -1l
endif else begin
    self.read_offset = -1l
...
pro gdlffdicom_shared::point_lun, read_offset
if (self.read_offset eq -1) then message, "File is closed"
if (read_offset mod 2 eq 1) then message, "No single byte offsets allowed"
self.read_offset = read_offset
end

pro gdlffdicom_shared::copy_lun, len
if (self.read_offset eq -1) then message, "File is closed"
shmmap, GET_NAME=segmentRead, OS_HANDLE=self.read_handle, OFFSET=self.read_offset, DIMENSIONS=len, TYPE=1
shmmap, GET_NAME=segmentWrite, OS_HANDLE=self.write_handle, OFFSET=self.write_offset, DIMENSIONS=len, TYPE=1
(shmvar(segmentWrite)) = shmvar(segmentRead)
self.read_offset = self.read_offset + len
self.write_offset = self.write_offset + len
end

pro gdlffdicom_shared::readu, value
if (self.read_offset eq -1) then message, "File is closed"
type = size(value,/TYPE)
;help, type
case (type) of 
    7: value = byte(value)
    13: if (self.read_offset mod 4 ne 0) then value = uintarr(n_elements(value)*2)
    3:  if (self.read_offset mod 4 ne 0) then value = uintarr(n_elements(value)*2)
    else:
endcase
shmmap, GET_NAME=segmentName, OS_HANDLE=self.read_handle, OFFSET=self.read_offset, TEMPLATE=[value]
value = shmvar(segmentName)

case (type) of 
13: begin
    if (self.read_offset mod 4 ne 0) then begin
;    print, value[0], FORMAT="(Z0)"
;    print, ishft(ulong(value[1]),16), FORMAT="(Z0)"
        nv = n_elements(value)
        value = ishft(ulong(value[2*indgen(nv/2)+1]),16) OR ulong(value[2*indgen(nv/2)])
;    print, value, FORMAT="(Z0)"
;    value = value +
;    print, value, FORMAT="(Z0)"
;        if (nv eq 2) then value = value[0]
    endif
    self.read_offset = self.read_offset + self -> nbytes(value)
end
3: begin
    if (self.read_offset mod 4 ne 0) then begin
;        print, value[0], FORMAT="(Z0)"
;        print, ishft(ulong(value[1]),16), FORMAT="(Z0)"
        nv = n_elements(value)
        value = long(ishft(ulong(value[2*indgen(nv/2)+1]),16) OR ulong(value[2*indgen(nv/2)]))
;    print, value, FORMAT="(Z0)"
;    value = value +
;    print, value, FORMAT="(Z0)"
;        if (nv eq 2) then value = value[0]
...
else: self.read_offset = self.read_offset + self -> nbytes(value)
endcase
 ;value = ulong(value[2*indgen(n_elements(value)/2)]) + 'FFFF'x*ulong(value[2*indgen(n_elements(value)/2+1)])
;help, value[0]
;print, value

end

pro gdlffdicom_shared::write_lun, value
if (self.read_offset eq -1) then message, "File is closed"
type = size(value,/TYPE)
if (type eq 7) then begin
    value = byte(value)
    if (n_elements(value) mod 2 eq 1) then value = [value,0b]
endif
shmmap, GET_NAME=segmentName, OS_HANDLE=self.read_handle, OFFSET=self.read_offset, TEMPLATE=[value]
var = shmvar(segmentName)
var[*] = value
self.read_offset = self.read_offset + self -> nbytes(value)
if (type eq 7) then value = string(value)
end

pro gdlffdicom_shared::writeu, value
if (self.write_offset eq -1) then message, "File is closed"
type = size(value,/TYPE)
if (type eq 7) then value = bytarr(value)
shmmap, GET_NAME=segmentName, OS_HANDLE=self.write_handle, OFFSET=self.write_offset, TEMPLATE=[value]
(shmvar(segmentName)) = value
self.write_offset = self.write_offset + self -> nbytes(value)
if (type eq 7) then value = string(value)
end

function gdlffdicom_shared::fsize
return, self.max_bytes
end

function gdlffdicom_shared::map, value, offset, count
if (self.read_offset eq -1) then message, "File is closed"
size = size(value)
value = MAKE_ARRAY(DIMENSION=[size[1:size[0]],count],TYPE=size[size[0]+1])
shmmap, GET_NAME=segmentName, OS_HANDLE=self.read_handle, OFFSET=offset, TEMPLATE=value
value = shmvar(segmentName)
...
pro gdlffdicom_shared__define, struct

struct = {gdlffdicom_shared, $
          lun: 0l, $
          lens: ptr_new(), $
          offsets: ptr_new(), $
          vrs: ptr_new(), $
