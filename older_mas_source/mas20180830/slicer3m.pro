; $Id: slicer3.pro,v 1.33 2000/06/13 20:45:22 chris Exp $
;
...
;      [ 0          , 0          , 0              , 1]]
;
;; This new quaternion matrix comes from
;; http://www.flipcode.com/documents/matrfaq.html
;; m = [[ 1-2*y^2-2*z^2, 2*(x*y-w*z), 2*(x*z+w*y), 0], $
;;      [ 2*(x*y+w*z), 1-2*y^2-2*z^2, 2*(y*z-w*x), 0], $
;;      [ 2*(x*z-w*y), 2*(y*z+w*x), 1-2*y^2-2*z^2, 0], $
;;      [ 0          , 0          , 0              , 1]]
;
;; From cw_arcball.pro
;; m = [[ w^2+x^2-y^2-z^2, 2*(x*y+w*z), 2*(x*z-w*y), 0], $
...
;
;;Data to normal
;FUNCTION data_to_normal, number
;
;    X0 = !X.S[0]
;    X1 = !X.S[1]
;    Y0 = !Y.S[0]
...
;normal = fltarr(3)
;
;    normal[0] = X0 + X1*number[0]
;    normal[1] = Y0 + Y1*number[1]
;    normal[2] = Z0 + Z1*number[2]
;
;
; RETURN, normal
;End
...
;
;;Data to Device
;FUNCTION data_to_device, number
;    X0 = !X.S[0]
;    X1 = !X.S[1]
;    Y0 = !Y.S[0]
;    Y1 = !Y.S[1]
...
;device_num = fltarr(3)
;
;    device_num[0] = Vx(X0 + X1*number[0])
;    device_num[1] = Vy(Y0 + Y1*number[1])
;    device_num[2] = number[2] ;No Z device transform
;
; RETURN, device_num
;End
;
;
;;Normal to device
;FUNCTION normal_to_device, number
;
;    Vx = !D.X_VSIZE
;    Vy = !D.Y_VSIZE
;
;device_num = fltarr(3)
;
;    device_num[0] = number[0]*Vx
;    device_num[1] = number[1]*Vy
;    device_num[2] = number[2] ;No Z device transform
;
; RETURN, device_num
;End
...
;
;;Normal to Data
;FUNCTION normal_to_data, number
;    X0 = !X.S[0]
;    X1 = !X.S[1]
;    Y0 = !Y.S[0]
;    Y1 = !Y.S[1]
...
;data = fltarr(3)
;
;    data[0] = (number[0]-X0)/X1
;    data[1] = (number[1]-Y0)/Y1
;    data[2] = (number[2]-Z0)/Z1
;
; RETURN, data
;End
;
;
;;Device to data
;FUNCTION device_to_data, number
;    X0 = !X.S[0]
;    X1 = !X.S[1]
;    Y0 = !Y.S[0]
;    Y1 = !Y.S[1]
...
;    data = fltarr(3)
;
;    data[0] = (number[0]/Vx-X0)/X1
;    data[1] = (number[1]/Vy-Y0)/Y1
;    data[2] = number[2] ;No Z device transform
;
; RETURN, data
;End
;
;
;;Device to normal
;FUNCTION device_to_normal, number
;
;    Vx = !D.X_VSIZE
;    Vy = !D.Y_VSIZE
;
;normal = fltarr(3)
;
;    normal[0] = number[0]/Vx
;    normal[1] = number[1]/Vy
;    normal[2] = number[2] ;No Z device transform
;
;
; RETURN, normal
;End
...
pro Viz3D_DiffColor_m, sViz3DColors
   COMPILE_OPT hidden
...
function Viz3D_LoadColor_m, CTAB=cTab, DIFFSHADE=diffShade
   COMPILE_OPT hidden
   ...
   Viz3D_DiffColor_m, sViz3DColors
...
function Viz3D_TransColor_m, sMainState, cIndex
      COMPILE_OPT hidden
...
function Viz3D_View_m, viewWin, $
   ...
   COMPILE_OPT hidden
   ;common mouseinfo,  mouse_x, mouse_y, mousedown, slider_rot1, slider_rot2, slider_rot3, rotation_transform
   ...
   if zScale ge 1.0 then zRange = FLOAT(zMax) * (zScale) else zRange = FLOAT(zMax)
   ;zRange = FLOAT(zMax) * (zScale > 1.0)

   ;maxRange = xRange > yRange > zRange
    if (xRange ge yRange) and (xRange ge zRange) then begin
       maxRange = xRange
    endif else if (yRange ge xRange) and (yRange ge zRange) then begin
       maxRange = yRange
    endif else maxRange = zRange


    x_element = xRange / maxRange
    y_element = yRange / maxRange
    z_element = zRange / maxRange
...
;if (maxRange_size[0] eq 0) then begin
;   xyzFac = [xRange, yRange, zRange] / maxRange
;
;endif else if (maxRange_size[0] eq 1) then begin
;
;   ; Maxrange is returning an array of one number not a single number
;   ;xyzFac = [xRange, yRange, zRange] / maxRange
...
; HS
   ; This is called the zyz convention
   ; http://www.easyspin.ethz.ch/eulerangles.html
...
;   ; HS 20071129
;   ; This is REALLY the last rotation, based on the rotation of mouse event in the 'view' mode
;    vTrans = TEMPORARY(vTrans) # rotation_transform
;
;             ; Going to update the rotation angles in the sliders
;
;               angle_y = -asin( rotation_transform[0,1]);        ;/* Calculate Y-axis angle */
;                 C =  cos( angle_y );
;                 angle_y *= !RADEG
;             ;
;                 if ( abs( C ) gt 0.005 ) then begin ;            /* Gimball lock? */
;
;                   trx      =  rotation_transform[2,2] / C;           ;/* No, so get X-axis angle */
;                   try      = -rotation_transform[2,1] / C;
;
;                   angle_x  = atan( try, trx ) * !RADEG;
;
;                   trx      =  rotation_transform[1,1] / C;            ;/* Get Z-axis angle */
;                   try      = -rotation_transform[0,1] / C;
;
;                   angle_z  = atan( try, trx ) * !RADEG;
;                 endif else begin                                ; /* Gimball lock has occurred */
;                   angle_x  = 0;                      ;/* Set X-axis angle to zero */
;
;                   trx      = rotation_transform[2,2];                 ;/* And calculate Z-axis angle */
;                   try      = rotation_transform[2,1];
;
;                   angle_z  = atan( try, trx ) * !RADEG;
;                 endelse
;
...
;>>>>>>>>>>>>>>>>>>>>>>>>
; BT 20080326
; allows zoom greater than corner view
...
pro Viz3D_FillDepth_m, sMainState
...
function Viz3D_GetData_m, sMainState, curData
...
pro Viz3D_PutData_m, data3D, sMainState, curData
...
function Viz3D_ScaleData_m, data3D, sMainState
...
pro Viz3D_DrawSliceOblique_m, obliqCenter, obliqNormal, fillColor, $
...
pro Viz3D_SliceShow_m, sMainState

   COMPILE_OPT hidden

   fillColor = Viz3D_TransColor_m(sMainState, sMainState.sSliceState.fillColor)
   edgeColor = Viz3D_TransColor_m(sMainState, sMainState.sSliceState.edgeColor)
   normColor = Viz3D_TransColor_m(sMainState, sMainState.sSliceState.normColor)
   backColor = Viz3D_TransColor_m(sMainState, sMainState.backColor)
   ...
   case (sMainState.sSliceState.planeMode*sMainState.sSliceState.orthoDir) of
      0: begin ; Oblique
         Viz3D_DrawSliceOblique_m, sMainState.sSliceState.obliqCenter, $
   ...
   Viz3D_DrawCube_m, sMainState, /SKIPBACK, /SKIPAXIS, /DIRECT
...
pro Viz3D_DrawCube_m, sMainState, SKIPCUBE=skipCube, SKIPAXIS=skipAxis, $
   ...
   cubeColor = Viz3D_TransColor_m(sMainState, sMainState.cubeColor)
   axisColor = Viz3D_TransColor_m(sMainState, sMainState.axisColor)
...
;******************************************************************************
pro Viz3D_DrawData_m, sMainState

   COMPILE_OPT hidden

   SET_PLOT, 'Z'
   Viz3D_DrawCube_m, sMainState, /SKIPFRONT
   img = TVRD()
   SET_PLOT, sMainState.screenDevice
   WSET, sMainState.pixWin
   TV, TEMPORARY(img)
   Viz3D_DrawCube_m, sMainState, /SKIPBACK
   EMPTY
   WSET, sMainState.mainWin
...
pro Viz3D_BlockShow_m, sMainState, DIRECT=directDraw
   ...
   blockColor = $
      Viz3D_TransColor_m(sMainState, sMainState.sBlockState.blockColor)

   ; Draw the cube outline in the current window.
   if not(KEYWORD_SET(directDraw)) then begin
      ERASE
      Viz3D_DrawCube_m, sMainState, /DIRECT, /AXIS
   endif else begin

      c1Color = Viz3D_TransColor_m(sMainState, sMainState.sBlockState.c1Color)
      c2Color = Viz3D_TransColor_m(sMainState, sMainState.sBlockState.c2Color)
...
pro Viz3D_ViewShow_m, sMainState
...
pro Viz3D_SurfShow_m, sMainState, CALC_HIST=calcHist

   COMPILE_OPT hidden

   WSET, sMainState.sSurfState.surfWin

   tColor = Viz3D_TransColor_m(sMainState, sMainState.sSurfState.tColor)
   histColor = Viz3D_TransColor_m(sMainState, sMainState.sSurfState.histColor)
   axisColor = Viz3D_TransColor_m(sMainState, sMainState.sSurfState.axisColor)
   ...
   *(sMainState.sSurfState.hSurfThresh) = surfThresh
   ...
   if (KEYWORD_SET(calcHist)) then begin
      data3D = Viz3D_GetData_m(sMainState)
      bSize = (hTh - lTh) / 200.0
      ...
      Viz3D_PutData_m, data3D, sMainState
...
pro Viz3D_ThreshShow_m, sMainState, DYNAMIC=dynUpdate

   COMPILE_OPT hidden

   WSET, sMainState.sThreshState.threshWin
   lowColor = Viz3D_TransColor_m(sMainState, sMainState.sThreshState.lowColor)
   highColor = Viz3D_TransColor_m(sMainState, sMainState.sThreshState.highColor)
   transColor = $
      Viz3D_TransColor_m(sMainState, sMainState.sThreshState.transColor)
   histColor = Viz3D_TransColor_m(sMainState, sMainState.sThreshState.histColor)
   axisColor = Viz3D_TransColor_m(sMainState, sMainState.sThreshState.axisColor)
   backColor = Viz3D_TransColor_m(sMainState, 0)
...
pro Viz3D_ProfShow_m, sMainState, DIRECT=directDraw

   COMPILE_OPT hidden

   lineColor = Viz3D_TransColor_m(sMainState, sMainState.sProfState.lineColor)
   axisColor = Viz3D_TransColor_m(sMainState, sMainState.sProfState.axisColor)
   markColor1 = Viz3D_TransColor_m(sMainState, sMainState.sProfState.markColor1)
   markColor2 = Viz3D_TransColor_m(sMainState, sMainState.sProfState.markColor2)
...
function Viz3D_TranspValu_m, sMainState, planeDir
   COMPILE_OPT hidden
...
function Viz3D_Add_Graphic_m, sMainState, graphic, NO_COPY=noCopy
...
pro Viz3D_OrthoPlaneDraw_m, sMainState, SKIP_ADD=skipAdd, SKIP_DRAW=skipDraw
   ...
   dataPlane = Viz3D_ScaleData_m(Temporary(dataPlane), sMainState)
   ...
   if (sMainState.sSliceState.drawMode) then begin ; Expose mode
      ...
      POLYFILL, x, y, z, PATTERN=dataPlane, /T3D, /NORMAL, $
         ...
         TRANSPARENT=Viz3D_TranspValu_m(sMainState, sliceDir-1), $
   ...
   if (NOT(KEYWORD_SET(skipAdd))) then begin
      ...
      h = Viz3D_Add_Graphic_m(sMainState, graphic, /NO_COPY)
   endif
...
pro Viz3D_ObliqPlaneDraw_m, sMainState, vertPlane, vert3D, SKIP_ADD=skipAdd, $
       SKIP_DRAW=skipDraw
   ...
   if (sMainState.sSliceState.drawMode) then begin ; Expose mode
      ...
      POLYFILL, vert3D, PATTERN=dataPlane, /T3D, /NORMAL, $
         IMAGE_INTERP=sMainState.interpMode, $
         TRANSPARENT=Viz3D_TranspValu_m(sMainState, maxInd), $
...
pro Viz3D_BlockDraw_m, sMainState, SKIP_ADD=skipAdd, SKIP_DRAW=skipDraw
   ...
   if (sMainState.sBlockState.blockMode) then begin ; Add mode.
      ...
      dim2 = (y2 - y1)
      faceDir = 2
      subImg = Viz3D_ScaleData_m(REFORM(data3D[x1:x2, y1:y2, z1]), sMainState)
      ...
      POLYFILL, [x1,x2,x2,x1], [y1,y1,y2,y2], [z1,z1,z1,z1], PATTERN=subImg, $
         ...
         TRANSPARENT=Viz3D_TranspValu_m(sMainState, faceDir), $
         IMAGE_COORD=[[0,0],[dim1,0],[dim1,dim2],[0,dim2]]
      subImg = Viz3D_ScaleData_m(REFORM(data3D[x1:x2, y1:y2, z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      dim2 = (z2 - z1)
      faceDir = 1
      subImg = Viz3D_ScaleData_m(REFORM(data3D[x1:x2, y1, z1:z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      POLYFILL, [x1,x2,x2,x1], [y1,y1,y1,y1], [z1,z1,z2,z2], PATTERN=subImg, $
         ...
         TRANSPARENT=Viz3D_TranspValu_m(sMainState, faceDir), $
         IMAGE_COORD=[[0,0],[dim1,0],[dim1,dim2],[0,dim2]]
      subImg = Viz3D_ScaleData_m(REFORM(data3D[x1:x2, y2, z1:z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      dim2 = (z2 - z1)
      faceDir = 0
      subImg = Viz3D_ScaleData_m(REFORM(data3D[x1, y1:y2, z1:z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      POLYFILL, [x1,x1,x1,x1], [y1,y2,y2,y1], [z1,z1,z2,z2], PATTERN=subImg, $
         ...
         TRANSPARENT=Viz3D_TranspValu_m(sMainState, faceDir), $
         IMAGE_COORD=[[0,0],[dim1,0],[dim1,dim2],[0,dim2]]
      subImg = Viz3D_ScaleData_m(REFORM(data3D[x2, y1:y2, z1:z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 2
         subImg = $
            Viz3D_ScaleData_m(REFORM(data3D[x1:x2, y1:y2, z1]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 1
         subImg = $
            Viz3D_ScaleData_m(REFORM(data3D[x1:x2, y1, z1:z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 0
         subImg = $
            Viz3D_ScaleData_m(REFORM(data3D[x1, y1:y2, z1:z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 2
         subImg = $
            Viz3D_ScaleData_m(REFORM(data3D[x1:x2, y1:y2, z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 1
         subImg = $
            Viz3D_ScaleData_m(REFORM(data3D[x1:x2, y2, z1:z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 0
         subImg = $
            Viz3D_ScaleData_m(REFORM(data3D[x2, y1:y2, z1:z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
...
pro Viz3D_SurfDraw_m, sMainState, hVertList, hPolyList, hColorList, $
   ...
   if (KEYWORD_SET(calcSurf)) then begin
      ...
      if (sMainState.curData eq sMainState.sSurfState.curShade) then begin
         ...
         data3D = $
            Viz3D_GetData_m(sMainState, sMainState.sSurfState.curShade)
         shadeData = data3D
...
pro Viz3D_ProjDraw_m, sMainState, SKIP_ADD=skipAdd, SKIP_DRAW=skipDraw
   ...
   data3D_raw = Viz3D_GetData_m(sMainState)
   data3D = data3D_raw
   Viz3D_PutData_m, data3D_raw, sMainState
   data3D = Viz3D_ScaleData_m(data3D, sMainState)
...
pro Viz3D_ProbeDraw_m, sMainState

   COMPILE_OPT hidden

   lineColor = Viz3D_TransColor_m(sMainState, sMainState.sProbeState.lineColor)
...
pro Viz3D_ProfDraw_m, sMainState
   ...
   while (PTR_VALID(hID)) do begin
      ...
      case gType of
         'VIZ3D_ORTHO_PLANE': begin
            ...
            Viz3D_OrthoPlaneDraw_m, TEMPORARY(altState), /SKIP_ADD, $
               /SKIP_DRAW
         ...
         'VIZ3D_OBLIQ_PLANE': begin
            ...
            Viz3D_ObliqPlaneDraw_m, TEMPORARY(altState), vertPlane, vert3D, $
               /SKIP_ADD, /SKIP_DRAW
         ...
         'VIZ3D_BLOCK': begin
            ...
            Viz3D_BlockDraw_m, TEMPORARY(altState), /SKIP_ADD, $
               /SKIP_DRAW
         ...
         'VIZ3D_SURF': begin
            ...
            Viz3D_SurfDraw_m, TEMPORARY(altState), graphic.hVertList, $
         ...
         'VIZ3D_PROJ': begin
            ...
            Viz3D_ProjDraw_m, TEMPORARY(altState), /SKIP_ADD, /SKIP_DRAW
...
function Viz3D_XYZPos_m, dX, dY, hDepth, hImage, invTrans, ON_FACE=onFace
   ...
   case uVal of
      'wMainDraw': begin ; Big view window.
         ...
         if ((event.press eq 1) or $
            ...
            ; cursor position.
            xyzPos = Viz3D_XYZPos_m(event.x, event.y, sMainState.hFrontDepth, $
            ...
            if (onFace gt 0) then begin
               ...
               if (sMainState.sSliceState.planeMode eq 1) then begin
                  ...
                  case sMainState.sSliceState.orthoDir of
                     1: begin ; X
                        Plots, xyzPos[0], [0,1,1,0,0], [0,0,1,1,0], /NORMAL, $
                           /T3D, COLOR=Viz3D_TransColor_m(sMainState, $
                  ...
                  edgeColor = Viz3D_TransColor_m(sMainState, $
                                 sMainState.sSliceState.edgeColor)
                  normColor = Viz3D_TransColor_m(sMainState, $
         ...
         if (event.release eq 1) then begin ; Cleanup and draw slice.
            ...
            if (TOTAL(ABS(xyzPos)) gt 0.0) then begin
               ...
               if (sMainState.sSliceState.planeMode eq 1) then begin
                  ...
                  Viz3D_OrthoPlaneDraw_m, sMainState
                  ...
                  if (sMainState.sSliceState.obliqMove eq 0) then begin
                     ...
                     endcase
                  endelse
                  fillColor = Viz3D_TransColor_m(sMainState, $
                                 sMainState.sSliceState.fillColor)
                  edgeColor = Viz3D_TransColor_m(sMainState, $
                                 sMainState.sSliceState.edgeColor)
                  normColor = Viz3D_TransColor_m(sMainState, $
                                 sMainState.sSliceState.normColor)
                  backColor = Viz3D_TransColor_m(sMainState, $
      ...
      'wSliceObliqGoBttn': begin ; Draw the slice.
         ...
         Viz3D_ObliqPlaneDraw_m, sMainState, vertPlane, vert3D
      end
      else:
   endcase
...
pro Viz3D_BlockEvent_m, event
   ...
   case uVal of
      'wMainDraw': begin ; Big draw window, move block.
         ...
         DEVICE, COPY=[0, 0, sMainState.winX, sMainState.winY, 0, 0, $
                       sMainState.pixWin]
         Viz3D_BlockShow_m, sMainState, /DIRECT

         if (event.release ge 1) then begin ; Cleanup.
            Viz3D_BlockShow_m, sMainState
         endif
...
pro Viz3D_SurfEvent_m, event
   ...
   case uVal of
      ...
      'wSurfGoBttn': Viz3D_SurfDraw_m, sMainState, /CALC_SURF
   endcase
...
pro Viz3D_ProjectEvent_m, event
   COMPILE_OPT hidden
   ...
   case uVal of
      ...
      'wProjGoBttn': Viz3D_ProjDraw_m, sMainState
   endcase
      ...
      'wThreshDraw': begin ; Set new threshold value via draw widget.
         if (event.release le 0) then begin
            ...
            if (event.release eq 1) then begin
               WIDGET_CONTROL, /HOURGLASS
               Viz3D_SurfShow_m, sMainState, /CALC_HIST
            endif
         endelse

         Viz3D_ThreshShow_m, sMainState, /DYNAMIC
      end
...
pro Viz3D_ProfileEvent_m, event
      ...
      'wMainDraw': begin ; Big draw window.
         ...
         if ((event.press ge 1) or $
            ...
            if (sMainState.sProfState.frontBack) then $
               xyzPos = Viz3D_XYZPos_m(event.x, event.y, $
            ...
            if (onFace gt 0) then begin ; On cube, draw profile.
               ...
               if (onFace ne sMainState.sProfState.onFace) then begin
                  ...
                  Viz3D_ProfShow_m, sMainState
               endif else Viz3D_ProfShow_m, sMainState, /DIRECT

               Viz3D_ProfDraw_m, sMainState
...
; HS 20070123
; Editing this function to add rotation when the user clicks on the screen

pro Viz3D_ViewEvent_m, event
   COMPILE_OPT hidden
   ...
   Viz3D_CleanView_m, sMainState, /SKIP_BUFFER

   redraw_flag = 0
   case uVal of
      'wMainDraw':  ; Big view window, do nothing.

      ;end ; This is the end of the case statement for wMainDraw
      ...
      'wViewDispBttn': begin
         ...
         Viz3D_CleanBuffer_m, sMainState
      end
   endcase

   if (redraw_flag) then begin
      sMainState.sViewState = $
         Viz3D_View_m(sMainState.sViewState.viewWin, $
...
pro Viz3D_DiffShadeEvent_m, event
   COMPILE_OPT hidden
...
;                    PTR_FREE, (*(sMainState.hDisplayList))[i]
      ...
      'wSaveTiffBttn': begin ; Save tiff image.
        if (LMGR(/DEMO)) then begin
         ...
         if (sMainState.sColorState.displayBits eq 24) then begin
            WRITE_TIFF, tiffFile[0], TVRD(/TRUE)
         endif else begin
            image = TVRD()
            TVLCT, cR, cG, cB, /GET
      ...
      'wOptionsBttn': begin ; Options.
         ...
         XMANAGER, 'Viz3D_Event_m', wOptBase, $
            EVENT_HANDLER='Viz3D_OptEvent_m'
         WIDGET_CONTROL, /HOURGLASS
         ...
         if (sOptState.Ok) then begin ; User clicked Ok.
            ...
            if ((sOptState.winXY ne sMainState.winX) or $
               ...
               Viz3D_FillDepth_m, sMainState
               Viz3D_Redraw_m, sMainState
            endif else begin
      ...
      'wColResetBttn': begin ; Reset Colors.
         sMainState.sColorState = Viz3D_LoadColor_m(DIFFSHADE=20)
         WSET, sMainState.mainWin
         Viz3D_DrawData_m, sMainState
      end
      ...
      'wColDiffBttn': begin ; Differential shading.
         ...
         XMANAGER, 'Viz3D_Event_m', diffBase, $
            EVENT_HANDLER='Viz3D_DiffShadeEvent_m'
         ...
         case event.index of
            ...
            3: begin ; Projection mode.
               if (sMainState.actMode ne 4) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='Viz3D_ProjectEvent_m'
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
               endif
            end
            ...
            5: begin ; Profile mode.
               if (sMainState.actMode ne 5) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='Viz3D_ProfileEvent_m'
                  Viz3D_ProfShow_m, sMainState
                  Viz3D_ProfDraw_m, sMainState
                  sMainState.cleanupView = 1
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
...
pro Viz3D_CleanView_m, sMainState, SKIP_BUFFER=skipBuffer
...
pro Viz3D_CleanBuffer_m, sMainState
   COMPILE_OPT hidden
...
pro Viz3D_KillDraw_m, wDrawBase
...
pro Viz3D_KillMain_m, wMainBase
...
pro Slicer3m, hData3D, DATA_NAMES=dataNames, DETACH=detachView, $
             GROUP_LEADER=groupLead, MODAL=modal, ASPECT_RATIO=aspect_ratio, SELECTED_SLICES=selected_slices


; Create a common block to store the mouse position for click events and image transformation
; common mouseinfo, mouse_x, mouse_y, mousedown, slider_rot1, slider_rot2, slider_rot3, rotation_transform


    ; Let's generate an identity matrix for the transformation when the user rotates.

   ident4 = FLTARR(4, 4)
   ident4[[0,5,10,15]] = 1.0
    rotation_transform = ident4
 ...
 ; Screen size enlarged by E. Ozarslan (denominators used to read 2 and it wasn't "floored".)
   winX = ((floor(screenSize[0] / 1.6)) < 640) > 320 ; View window size.
   winY = ((floor(screenSize[1] / 1.6)) < 640) > 320 ; View window size.
   winX = 8 * ((winX < winY) / 8)
   winY = winX
   winX = screenSize[1]*.85
   winY = winX
...
; Generic widget_base keywords for modal or nonmodal
    extra = {TITLE:'IDL 3D Data Visualizer (Slicer3 Modified)', $
...
;                     UVALUE='wAnimateBttn')
   ...
   if (detachBase and NOT runModal) then $
      ...
      wDrawBase = WIDGET_BASE(wMainBase, UVALUE=wMainBase, /COLUMN, $
         SPACE=1, XPAD=1, YPAD=1, EVENT_PRO='Viz3D_SliceEvent_m')
...
; Adding the slider to a global variable to be able to control it
    slider_rot1 = wRot1Slid
   ...
   slider_rot3 = wRot3Slid

   WIDGET_CONTROL, wRot3Drop, SET_DROPLIST_SELECT=1

   wZoomBase = WIDGET_BASE(wViewBase, /ROW, SPACE=1, XPAD=1, YPAD=1, $
                           /FRAME)
;>>>>>>>>>>>>>>>
;; BT 20080326 Added larger max zoom
;   wZoomSlid = WIDGET_SLIDER(wZoomBase, Min=50, Max=100, VALUE=50, $
;                  XSIZE=136, UVALUE='wZoomSlid', /DRAG)

   wZoomSlid = WIDGET_SLIDER(wZoomBase, Min=50, Max=300, VALUE=50, $
                  XSIZE=136, UVALUE='wZoomSlid', /DRAG)
;<<<<<<<<<<<<<<<
...
;   wid = WIDGET_LABEL(wPerspBase, VALUE='Persp.')
   ...
   sColorState = Viz3D_LoadColor_m(DIFFSHADE=20)

   ; View state.
   sViewState = Viz3D_View_m(viewWin, ANG1=ang1, ANG2=ang2, ANG3=ang3, $
      DIR1='Z', DIR2='X', DIR3='Y', ZOOM=0.90, XMAX=szData[1]-1L, $
...
;----------> Default value of orthoDir was 1. Changed by E. Ozarslan
   ...
   if (runModal) then begin
      ...
      XMANAGER, 'Slicer3', wMainBase, EVENT_HANDLER='Viz3D_Event_m', $
         CLEANUP='Viz3D_KillMain_m'
      ...
      if (detachBase) then $
         XMANAGER, 'Slicer3', wDrawBase, /JUST_REG, $
            EVENT_HANDLER='Viz3D_SliceEvent_m', CLEANUP='Viz3D_KillDraw_m'
