...
;
; Purpose of subroutine: Data selection GUI which allows which arrays to fit for analysis (T1,T2,ADC & ADT, gradient eddy current field)
;
; Editing Information:

pro select_data_gui, index = index, paired = paired
common scan_data
 
adim = project.imndarray[index].adim
if keyword_set(paired) then adim/=2
...
'misc_buttons' : begin                 
                 ...
                 'Reset'   : begin
                             widget_control, info.dat_button_grp, get_value = temp
                             widget_control, info.dat_button_grp, set_value= make_array(n_elements(temp),value = 1)   
                             end                                                                       
                 'Close'   : begin
                             widget_control, info.dat_button_grp, get_value = temp
                             widget_control, info.dat_button_grp, set_value= make_array(n_elements(temp),value = 1) 
