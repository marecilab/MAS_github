...
pro roi3d_ortho_grabber, sMainState

    COMPILE_OPT hidden, idl2

common share1, nnn, slices, nmax, over
    if PTR_VALID(sMainState.hDisplayList) then begin
	  ...
	  dims=[sMainState.sViewState.xMax, sMainState.sViewState.yMax $
						  ,	sMainState.sViewState.zMax]
	  if slicedir eq 1 then slices[*, nnn]=[1.,0.,0.,slicepos,0.5,0.5]
	  if slicedir eq 2 then slices[*, nnn]=[0.,1.,0.,0.5,slicepos,0.5]
	  if slicedir eq 3 then slices[*, nnn]=[0.,0.,1.,0.5,0.5,slicepos]

	  slices[3:5, nnn]=(dims+1)*slices[3:5, nnn]
	text=strarr(nnn+1)
	  for i=0,nnn do begin
	    if slices[0,i] eq 1. then text[i]='	x	'+strtrim(string(round(slices[3,i])),2)
	    if slices[1,i] eq 1. then text[i]='	y	'+strtrim(string(round(slices[4,i])),2)
	    if slices[2,i] eq 1. then text[i]='	z	'+strtrim(string(round(slices[5,i])),2)
	   ;  text[i]=' ( '+strmid(strtrim(string(slices[0,i]),2),0,7)+', '+ $
	;	strmid(strtrim(string(slices[1,i]),2),0,7)+', '+$
	;	strmid(strtrim(string(slices[2,i]),2),0,7)+' )		( '+$
	;	strmid(strtrim(string(slices[3,i]),2),0,7)+', '+$
	;	strmid(strtrim(string(slices[4,i]),2),0,7)+', '+$
	;	strmid(strtrim(string(slices[5,i]),2),0,7)+' )'
	  endfor
   	WIDGET_CONTROL, sMainState.wText, SET_VALUE=text
	nnn=nnn+1
...
; ******************************************
; Added by HS 20070831
; Based on Evren's 'roi3d_ortho_grabber'
; Grabs a slice based on the value from a textbox
; ******************************************

pro roi3d_manual_grabber, sMainState
    ...
    if PTR_VALID(sMainState.hDisplayList) then begin
      ;if N_ELEMENTS(*(sMainState.hDisplayList)) GT 0 then begin
	  slicedir=sMainState.SSliceState.OrthoDir
		slices_shown = sMainState.hDisplayList


	  ;slicepos=SMainState.SSliceState.Orthopos

	  WIDGET_CONTROL, sMainState.w_manual_input_textbox, GET_VALUE = temp
	  WIDGET_CONTROL, sMainState.w_manual_input_textbox, SET_VALUE = ''
	  ;slicepos = round(float(temp))
	  slicepos = temp

	  dims=[sMainState.sViewState.xMax, sMainState.sViewState.yMax $
						  ,	sMainState.sViewState.zMax]

	  if slicedir eq 1 then begin

	  	slices[*, nnn]=[1.,0.,0.,slicepos,0.5,0.5]
	  	;scaled_slice = slicepos/(dims[0]+1)

	  endif else if slicedir eq 2 then begin

	  	slices[*, nnn]=[0.,1.,0.,0.5,slicepos,0.5]
	  	;scaled_slice = slicepos/(dims[1]+1)

	  endif else if slicedir eq 3 then begin

	  	slices[*, nnn]=[0.,0.,1.,0.5,0.5,slicepos]
	  	;scaled_slice = slicepos/(dims[2]+1)
...
;*******************************************************************************
; Added by E. Ozarslan
pro roi3d_obliq_grabber, sMainState
...
common share1, nnn, slices, nmax, over
    if PTR_VALID(sMainState.hDisplayList) then begin
	  ...
	  if norm(sliceNormal) ne 0. then $
				sliceNormal=sliceNormal/norm(sliceNormal)
	  sliceCenter = sMainState.sSliceState.obliqCenter
	  dims=[sMainState.sViewState.xMax, sMainState.sViewState.yMax $
								  ,	sMainState.sViewState.zMax]
	  sliceCenter=sliceCenter*dims
	  slices[*,nnn]=[sliceNormal, sliceCenter]
	text=strarr(nnn+1)
	  for i=0,nnn do begin
	    text[i]=' ( '+strmid(strtrim(string(slices[0,i]),2),0,7)+', '+ $
		strmid(strtrim(string(slices[1,i]),2),0,7)+', '+$
...
;******************************************************************************
;Added by E.Ozarslan
pro roi3d_slice_deleter, sMainState
...
common share1, nnn, slices, nmax, over
    if PTR_VALID(sMainState.hDisplayList) then begin
	nnn=0
	slices=Replicate(0., 6, nmax)
	WIDGET_CONTROL, sMainState.wText, $
	      SET_VALUE=''
    endif else hID = PTR_NEW() ;null pointer, not valid
end
...
;******************************************************************************
;Added by E. Ozarslan
pro roi3d_grab_n_go, sMainState
...
if nnn gt 0 then begin
    if PTR_VALID(sMainState.hDisplayList) then begin
	slices_final=make_array(6,nnn)
	slices_final=slices[*,0:nnn-1]
	slices=slices_final
	over=1
    endif else hID = PTR_NEW() ;null pointer, not valid
endif
...
pro roi3d_Viz3D_DiffColor, sViz3DColors
   COMPILE_OPT hidden, idl2
...
function roi3d_Viz3D_LoadColor, CTAB=cTab, DIFFSHADE=diffShade
   COMPILE_OPT hidden, idl2
   ...
   roi3d_Viz3D_DiffColor, sViz3DColors
...
function roi3d_Viz3D_TransColor, sMainState, cIndex
      COMPILE_OPT hidden, idl2
...
function roi3d_Viz3D_View, viewWin, $
...
; This next line was causing a problem because maxRange was an array of one element and the
; division would create a one element answer.

   ;xyzFac = [xRange, yRange, zRange] / maxRange
   xyzFac = [xRange, yRange, zRange] / maxRange[0]
   m4X4 = ident4
   m4X4[0,0] = xyzFac[0]
   m4X4[1,1] = xyzFac[1]
   m4X4[2,2] = xyzFac[2]
...
pro roi3d_Viz3D_FillDepth, sMainState
...
function roi3d_Viz3D_GetData, sMainState, curData
...
pro roi3d_Viz3D_PutData, data3D, sMainState, curData
...
function roi3d_Viz3D_ScaleData, data3D, sMainState
...
pro roi3d_Viz3D_DrawSliceOblique, obliqCenter, obliqNormal, fillColor, $
...
pro roi3d_Viz3D_SliceShow, sMainState

   COMPILE_OPT hidden, idl2

   fillColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sSliceState.fillColor)
   edgeColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sSliceState.edgeColor)
   normColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sSliceState.normColor)
   backColor = roi3d_Viz3D_TransColor(sMainState, sMainState.backColor)
   ...
   case (sMainState.sSliceState.planeMode*sMainState.sSliceState.orthoDir) of
      0: begin ; Oblique
         roi3d_Viz3D_DrawSliceOblique, sMainState.sSliceState.obliqCenter, $
   ...
   roi3d_Viz3D_DrawCube, sMainState, /SKIPBACK, /SKIPAXIS, /DIRECT
...
pro roi3d_Viz3D_DrawCube, sMainState, SKIPCUBE=skipCube, SKIPAXIS=skipAxis, $
   ...
   cubeColor = roi3d_Viz3D_TransColor(sMainState, sMainState.cubeColor)
   axisColor = roi3d_Viz3D_TransColor(sMainState, sMainState.axisColor)
...
;******************************************************************************
pro roi3d_Viz3D_DrawData, sMainState

   COMPILE_OPT hidden, idl2

   SET_PLOT, 'Z'
   roi3d_Viz3D_DrawCube, sMainState, /SKIPFRONT
   img = TVRD()
   SET_PLOT, sMainState.screenDevice
   WSET, sMainState.pixWin
   TV, TEMPORARY(img)
   roi3d_Viz3D_DrawCube, sMainState, /SKIPBACK
   EMPTY
   WSET, sMainState.mainWin
...
; Inside of this procedure is commented in the earlier slice selection code
; E. Ozarslan
pro roi3d_Viz3D_BlockShow, sMainState, DIRECT=directDraw
   ...
   blockColor = $
      roi3d_Viz3D_TransColor(sMainState, sMainState.sBlockState.blockColor)

   ; Draw the cube outline in the current window.
   if not(KEYWORD_SET(directDraw)) then begin
      ERASE
      roi3d_Viz3D_DrawCube, sMainState, /DIRECT, /AXIS
   endif else begin

      c1Color = roi3d_Viz3D_TransColor(sMainState, sMainState.sBlockState.c1Color)
      c2Color = roi3d_Viz3D_TransColor(sMainState, sMainState.sBlockState.c2Color)
...
pro roi3d_Viz3D_ViewShow, sMainState
...
; Inside of this procedure is commented in the earlier slice selection code
; E. Ozarslan
pro roi3d_Viz3D_SurfShow, sMainState, CALC_HIST=calcHist

   COMPILE_OPT hidden, idl2

   WSET, sMainState.sSurfState.surfWin

   tColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sSurfState.tColor)
   histColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sSurfState.histColor)
   axisColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sSurfState.axisColor)
   ...
   *(sMainState.sSurfState.hSurfThresh) = surfThresh
   ...
   if (KEYWORD_SET(calcHist)) then begin
      data3D = roi3d_Viz3D_GetData(sMainState)
      bSize = (hTh - lTh) / 200.0
      ...
      roi3d_Viz3D_PutData, data3D, sMainState
...
pro roi3d_Viz3D_ThreshShow, sMainState, DYNAMIC=dynUpdate

   COMPILE_OPT hidden, idl2

   WSET, sMainState.sThreshState.threshWin
   lowColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sThreshState.lowColor)
   highColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sThreshState.highColor)
   transColor = $
      roi3d_Viz3D_TransColor(sMainState, sMainState.sThreshState.transColor)
   histColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sThreshState.histColor)
   axisColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sThreshState.axisColor)
   backColor = roi3d_Viz3D_TransColor(sMainState, 0)
...
; Inside of this procedure is commented in the earlier slice selection code
; E. Ozarslan
pro roi3d_Viz3D_ProfShow, sMainState, DIRECT=directDraw

   COMPILE_OPT hidden, idl2

   lineColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sProfState.lineColor)
   axisColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sProfState.axisColor)
   markColor1 = roi3d_Viz3D_TransColor(sMainState, sMainState.sProfState.markColor1)
   markColor2 = roi3d_Viz3D_TransColor(sMainState, sMainState.sProfState.markColor2)
...
function roi3d_Viz3D_TranspValu, sMainState, planeDir
   COMPILE_OPT hidden, idl2
...
function roi3d_Viz3D_Add_Graphic, sMainState, graphic, NO_COPY=noCopy
...
pro roi3d_Viz3D_OrthoPlaneDraw, sMainState, SKIP_ADD=skipAdd, SKIP_DRAW=skipDraw
   ...
   dataPlane = roi3d_Viz3D_ScaleData(Temporary(dataPlane), sMainState)
   ...
   if (sMainState.sSliceState.drawMode) then begin ; Expose mode
      ...
      POLYFILL, x, y, z, PATTERN=dataPlane, /T3D, /NORMAL, $
         ...
         TRANSPARENT=roi3d_Viz3D_TranspValu(sMainState, sliceDir-1), $
   ...
   if (NOT(KEYWORD_SET(skipDraw))) then $
      roi3d_Viz3D_DrawData, sMainState
...
pro roi3d_Viz3D_ObliqPlaneDraw, sMainState, vertPlane, vert3D, SKIP_ADD=skipAdd, $
       SKIP_DRAW=skipDraw
   ...
   if (sMainState.sSliceState.drawMode) then begin ; Expose mode
      ...
      POLYFILL, vert3D, PATTERN=dataPlane, /T3D, /NORMAL, $
         IMAGE_INTERP=sMainState.interpMode, $
         TRANSPARENT=roi3d_Viz3D_TranspValu(sMainState, maxInd), $
...
pro roi3d_Viz3D_BlockDraw, sMainState, SKIP_ADD=skipAdd, SKIP_DRAW=skipDraw
   ...
   if (sMainState.sBlockState.blockMode) then begin ; Add mode.
      ...
      dim2 = (y2 - y1)
      faceDir = 2
      subImg = roi3d_Viz3D_ScaleData(REFORM(data3D[x1:x2, y1:y2, z1]), sMainState)
      ...
      POLYFILL, [x1,x2,x2,x1], [y1,y1,y2,y2], [z1,z1,z1,z1], PATTERN=subImg, $
         ...
         TRANSPARENT=roi3d_Viz3D_TranspValu(sMainState, faceDir), $
         IMAGE_COORD=[[0,0],[dim1,0],[dim1,dim2],[0,dim2]]
      subImg = roi3d_Viz3D_ScaleData(REFORM(data3D[x1:x2, y1:y2, z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      dim2 = (z2 - z1)
      faceDir = 1
      subImg = roi3d_Viz3D_ScaleData(REFORM(data3D[x1:x2, y1, z1:z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      POLYFILL, [x1,x2,x2,x1], [y1,y1,y1,y1], [z1,z1,z2,z2], PATTERN=subImg, $
         ...
         TRANSPARENT=roi3d_Viz3D_TranspValu(sMainState, faceDir), $
         IMAGE_COORD=[[0,0],[dim1,0],[dim1,dim2],[0,dim2]]
      subImg = roi3d_Viz3D_ScaleData(REFORM(data3D[x1:x2, y2, z1:z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      dim2 = (z2 - z1)
      faceDir = 0
      subImg = roi3d_Viz3D_ScaleData(REFORM(data3D[x1, y1:y2, z1:z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      POLYFILL, [x1,x1,x1,x1], [y1,y2,y2,y1], [z1,z1,z2,z2], PATTERN=subImg, $
         ...
         TRANSPARENT=roi3d_Viz3D_TranspValu(sMainState, faceDir), $
         IMAGE_COORD=[[0,0],[dim1,0],[dim1,dim2],[0,dim2]]
      subImg = roi3d_Viz3D_ScaleData(REFORM(data3D[x2, y1:y2, z1:z2]), sMainState)
      subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                      TOP=sMainState.sColorState.nColor-1)
      subImg = TEMPORARY(subImg) + $
                  BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 2
         subImg = $
            roi3d_Viz3D_ScaleData(REFORM(data3D[x1:x2, y1:y2, z1]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 1
         subImg = $
            roi3d_Viz3D_ScaleData(REFORM(data3D[x1:x2, y1, z1:z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 0
         subImg = $
            roi3d_Viz3D_ScaleData(REFORM(data3D[x1, y1:y2, z1:z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 2
         subImg = $
            roi3d_Viz3D_ScaleData(REFORM(data3D[x1:x2, y1:y2, z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 1
         subImg = $
            roi3d_Viz3D_ScaleData(REFORM(data3D[x1:x2, y2, z1:z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
      ...
      if ((norm[2,1]-norm[2,0]) lt 0.0) then begin
         ...
         faceDir = 0
         subImg = $
            roi3d_Viz3D_ScaleData(REFORM(data3D[x2, y1:y2, z1:z2]), sMainState)
         subImg = BYTSCL(TEMPORARY(subImg), MIN=0B, MAX=255B, $
                         TOP=sMainState.sColorState.nColor-1)
         subImg = TEMPORARY(subImg) + $
                     BYTE(faceDir * sMainState.sColorState.nColor)
...
pro roi3d_Viz3D_SurfDraw, sMainState, hVertList, hPolyList, hColorList, $
   ...
   if (KEYWORD_SET(calcSurf)) then begin
      ...
      if (sMainState.curData eq sMainState.sSurfState.curShade) then begin
         ...
         data3D = $
            roi3d_Viz3D_GetData(sMainState, sMainState.sSurfState.curShade)
         shadeData = data3D
         roi3d_Viz3D_PutData, data3D, sMainState, sMainState.sSurfState.curShade
...
pro roi3d_Viz3D_ProjDraw, sMainState, SKIP_ADD=skipAdd, SKIP_DRAW=skipDraw
   ...
   data3D_raw = roi3d_Viz3D_GetData(sMainState)
   data3D = data3D_raw
   roi3d_Viz3D_PutData, data3D_raw, sMainState
   data3D = roi3d_Viz3D_ScaleData(data3D, sMainState)
...
pro roi3d_Viz3D_ProbeDraw, sMainState

   COMPILE_OPT hidden, idl2

   lineColor = roi3d_Viz3D_TransColor(sMainState, sMainState.sProbeState.lineColor)
...
pro roi3d_Viz3D_ProfDraw, sMainState
...
pro roi3d_Viz3D_Redraw, sMainState
   ...
   while (PTR_VALID(hID)) do begin
      ...
      case gType of
         'VIZ3D_ORTHO_PLANE': begin
            ...
            roi3d_Viz3D_OrthoPlaneDraw, TEMPORARY(altState), /SKIP_ADD, $
               /SKIP_DRAW
         ...
         'VIZ3D_OBLIQ_PLANE': begin
            ...
            roi3d_Viz3D_ObliqPlaneDraw, TEMPORARY(altState), vertPlane, vert3D, $
               /SKIP_ADD, /SKIP_DRAW
         ...
         'VIZ3D_BLOCK': begin
            ...
            roi3d_Viz3D_BlockDraw, TEMPORARY(altState), /SKIP_ADD, $
               /SKIP_DRAW
         ...
         'VIZ3D_SURF': begin
            ...
            roi3d_Viz3D_SurfDraw, TEMPORARY(altState), graphic.hVertList, $
         ...
         'VIZ3D_PROJ': begin
            ...
            roi3d_Viz3D_ProjDraw, TEMPORARY(altState), /SKIP_ADD, /SKIP_DRAW
...
; (the roi3d_Viz3D_FillDepth procedure computes them).
...
function roi3d_Viz3D_XYZPos, dX, dY, hDepth, hImage, invTrans, ON_FACE=onFace
...
pro roi3d_Viz3D_SliceEvent, event
      ...
      'wMainDraw': begin ; Big view window.
         ...
         if (event.release gt 1) then begin ; Toggle plane direction.
            if (sMainState.sSliceState.planeMode eq 1) then begin
               ...
               endcase
               roi3d_Viz3D_SliceShow, sMainState
            endif
         endif
         ...
         if ((event.press eq 1) or $
            ...
            ; cursor position.
            xyzPos = roi3d_Viz3D_XYZPos(event.x, event.y, sMainState.hFrontDepth, $
            ...
            if (onFace gt 0) then begin
               ...
               if (sMainState.sSliceState.planeMode eq 1) then begin
                  ...
                  case sMainState.sSliceState.orthoDir of
                     1: begin ; X
                        Plots, xyzPos[0], [0,1,1,0,0], [0,0,1,1,0], /NORMAL, $
                           /T3D, COLOR=roi3d_Viz3D_TransColor(sMainState, $
                  ...
                  edgeColor = roi3d_Viz3D_TransColor(sMainState, $
                                 sMainState.sSliceState.edgeColor)
                  normColor = roi3d_Viz3D_TransColor(sMainState, $
         ...
         if (event.release eq 1) then begin ; Cleanup and draw slice.
            ...
            if (TOTAL(ABS(xyzPos)) gt 0.0) then begin
               ...
               if (sMainState.sSliceState.planeMode eq 1) then begin
                  ...
                  roi3d_Viz3D_OrthoPlaneDraw, sMainState
                  ...
                  if (sMainState.sSliceState.obliqMove eq 0) then begin
                     ...
                     endcase
                  endelse
                  fillColor = roi3d_Viz3D_TransColor(sMainState, $
                                 sMainState.sSliceState.fillColor)
                  edgeColor = roi3d_Viz3D_TransColor(sMainState, $
                                 sMainState.sSliceState.edgeColor)
                  normColor = roi3d_Viz3D_TransColor(sMainState, $
                                 sMainState.sSliceState.normColor)
                  backColor = roi3d_Viz3D_TransColor(sMainState, $
...
; >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
; Added by E. Ozarslan
 'wSliceOrthoGrabBttn' : begin
roi3d_ortho_grabber, sMainState
end

'wSliceObliqGrabBttn' : begin
roi3d_obliq_grabber, sMainState
end

'wSliceDeleteBttn' : begin
roi3d_slice_deleter, sMainState
end

'wSliceDoneBttn' : begin
roi3d_grab_n_go, sMainState
mainID=event.top
...
; When pressing the Manual Grab Slice button

'w_manual_input_button' : begin
roi3d_manual_grabber, sMainState
end
...
; <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      ...
      'wSliceObliqGoBttn': begin ; Draw the slice.
         ...
         roi3d_Viz3D_ObliqPlaneDraw, sMainState, vertPlane, vert3D
      end
      else:
   endcase
...
; This flag is added by E. Ozarslan
FLAG:
...
pro roi3d_Viz3D_BlockEvent, event
   ...
   case uVal of
      'wMainDraw': begin ; Big draw window, move block.
         ...
         DEVICE, COPY=[0, 0, sMainState.winX, sMainState.winY, 0, 0, $
                       sMainState.pixWin]
         roi3d_Viz3D_BlockShow, sMainState, /DIRECT

         if (event.release ge 1) then begin ; Cleanup.
            roi3d_Viz3D_BlockShow, sMainState
         endif
      ...
      'wBlockGoBttn': begin ; Draw it.
         roi3d_Viz3D_BlockDraw, sMainState
      end
   endcase
...
pro roi3d_Viz3D_SurfEvent, event
   ...
   case uVal of
      ...
      'wSurfDraw': begin
         ...
         roi3d_Viz3D_SurfShow, sMainState
      ...
      'wSurfGoBttn': roi3d_Viz3D_SurfDraw, sMainState, /CALC_SURF
   endcase
...
pro roi3d_Viz3D_ProjectEvent, event
   COMPILE_OPT hidden, idl2
   ...
   case uVal of
      ...
      'wProjGoBttn': roi3d_Viz3D_ProjDraw, sMainState
   endcase
...
pro roi3d_Viz3D_ThreshEvent, event
      ...
      'wThreshDraw': begin ; Set new threshold value via draw widget.
         if (event.release le 0) then begin
            ...
            if (event.release eq 1) then begin
               WIDGET_CONTROL, /HOURGLASS
               roi3d_Viz3D_SurfShow, sMainState, /CALC_HIST
            endif
         endelse

         roi3d_Viz3D_ThreshShow, sMainState, /DYNAMIC
      end
...
pro roi3d_Viz3D_ProfileEvent, event
      ...
      'wMainDraw': begin ; Big draw window.
         ...
         if ((event.press ge 1) or $
            ...
            if (sMainState.sProfState.frontBack) then $
               xyzPos = roi3d_Viz3D_XYZPos(event.x, event.y, $
            ...
            if (onFace gt 0) then begin ; On cube, draw profile.
               ...
               if (onFace ne sMainState.sProfState.onFace) then begin
                  ...
                  roi3d_Viz3D_ProfShow, sMainState
               endif else roi3d_Viz3D_ProfShow, sMainState, /DIRECT

               roi3d_Viz3D_ProfDraw, sMainState
...
pro roi3d_Viz3D_ProbeEvent, event
         ...
         if ((event.press ge 1) or $
            ...
            if (zFront gt (-32765)) then begin
               ...
               if (nZ gt zBack) then begin
                  ...
                  roi3d_Viz3D_ProbeDraw, sMainState
...
pro roi3d_Viz3D_ViewEvent, event
   COMPILE_OPT hidden, idl2
   ...
   roi3d_Viz3D_CleanView, sMainState, /SKIP_BUFFER
   ...
   case uVal of
      ...
      'wViewDispBttn': begin
         ...
         roi3d_Viz3D_CleanBuffer, sMainState
      end
   endcase

   if (redraw_flag) then begin
      sMainState.sViewState = $
         roi3d_Viz3D_View(sMainState.sViewState.viewWin, $
      ...
      roi3d_Viz3D_ViewShow, sMainState
      sMainState.cleanupBuffer = 1
   endif
...
pro roi3d_Viz3D_DiffShadeEvent, event
   COMPILE_OPT hidden, idl2
...
pro roi3d_Viz3D_OptEvent, event
   COMPILE_OPT hidden, idl2
...
ON_IOERROR, IO_BAD
   ...
   if ((uVal ne 'wQuitBttn') AND (uVal ne 'wDelAllBttn')) then $
      roi3d_Viz3D_CleanView, sMainState
   ...
   case uVal of
      ...
      ;>>>>>>>>>>>>>>>>
      ; These are commented in the previous slice selection routine
      ; E. Ozarslan
                  *(sMainState.sSurfState.hRangeHist) = lRangeHist
...
;                    PTR_FREE, (*(sMainState.hDisplayList))[i]
                  ...
                  roi3d_Viz3D_DrawData, sMainState
                  sMainState.cleanupBuffer = 1
                  roi3d_Viz3D_ThreshShow, sMainState
                  roi3d_Viz3D_SurfShow, sMainState, /CALC_HIST
                  roi3d_Viz3D_CleanBuffer, sMainState
                  WIDGET_CONTROL, sMainState.wStatText, SET_VALUE=''
      ;>>>>>>>>>>>>>>>>
      ...
      'wSaveTiffBttn': begin ; Save tiff image.
        if (LMGR(/DEMO)) then begin
         ...
         tiffFile = DIALOG_PICKFILE(GROUP=event.top, /WRITE, FILTER='*.tif', $
                             FILE='ROI3d_slicer.tif') ; Filename changed. E. Ozarslan
         WIDGET_CONTROL, /HOURGLASS
      ...
      'wSaveSubsetBttn': begin ; Save subset.
        if (LMGR(/DEMO)) then begin
         ...
         dataFile = DIALOG_PICKFILE(GROUP=event.top, /WRITE, FILTER='*.dat', $
                             FILE='ROI3d_slicer.dat') ; Filename changed. E. Ozarslan
         WIDGET_CONTROL, /HOURGLASS
      ...
      'wOptionsBttn': begin ; Options.
         wOptBase = WIDGET_BASE(TITLE='ROI3d_slicer', GROUP_LEADER=event.top, $
                       /COLUMN, XPAD=1, YPAD=1, SPACE=1, /MODAL) ;Title Changed. E. Ozarslan
         ...
         XMANAGER, 'roi3d_Viz3D_Event', wOptBase, $
            EVENT_HANDLER='roi3d_Viz3D_OptEvent'
         WIDGET_CONTROL, /HOURGLASS
         ...
         if (sOptState.Ok) then begin ; User clicked Ok.
            ...
            if ((sOptState.winXY ne sMainState.winX) or $
               ...
               roi3d_Viz3D_FillDepth, sMainState
               roi3d_Viz3D_Redraw, sMainState
            endif else begin
      ...
      'wColResetBttn': begin ; Reset Colors.
         sMainState.sColorState = roi3d_Viz3D_LoadColor(DIFFSHADE=20)
         WSET, sMainState.mainWin
         roi3d_Viz3D_DrawData, sMainState
      end
      ...
      'wColDiffBttn': begin ; Differential shading.
         ...
         XMANAGER, 'roi3d_Viz3D_Event', diffBase, $
            EVENT_HANDLER='roi3d_Viz3D_DiffShadeEvent'
            ...
            0: begin ; Slice mode.
               if (sMainState.actMode ne 0) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='roi3d_Viz3D_SliceEvent'
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
               endif
            end
       ;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
       ;Commented in the previous slice selector.
       ;E. Ozarslan
            1: begin ; Block mode.
               if (sMainState.actMode ne 1) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='roi3d_Viz3D_BlockEvent'
                  roi3d_Viz3D_BlockShow, sMainState
                  roi3d_Viz3D_BlockShow, sMainState, /DIRECT
            ...
            2: begin ; Surface mode.
               if (sMainState.actMode ne 2) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='roi3d_Viz3D_SurfEvent'
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
               endif
            end
            3: begin ; Projection mode.
               if (sMainState.actMode ne 4) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='roi3d_Viz3D_ProjectEvent'
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
               endif
            end
     ;<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

     ;Below is renumbered in the previous slice selector.
     ;E. Ozarslan
            ...
            4: begin ; Threshold mode.
               if (sMainState.actMode ne 3) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='roi3d_Viz3D_ThreshEvent'
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
               endif
            end
       ;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
       ...
       ;E. Ozarslan
            5: begin ; Profile mode.
               if (sMainState.actMode ne 5) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='roi3d_Viz3D_ProfileEvent'
                  roi3d_Viz3D_ProfShow, sMainState
                  roi3d_Viz3D_ProfDraw, sMainState
                  sMainState.cleanupView = 1
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
            ...
            6: begin ; Probe mode.
               if (sMainState.actMode ne 6) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='roi3d_Viz3D_ProbeEvent'
                  sMainState.cleanupView = 1
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
                  roi3d_Viz3D_ProbeDraw, sMainState
               endif
        ...
        ;<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            ...
            7: begin ; View mode.
               if (sMainState.actMode ne 7) then begin
                  ...
                  WIDGET_CONTROL, sMainState.wDrawBase, $
                     EVENT_PRO='roi3d_Viz3D_ViewEvent'
                  WIDGET_CONTROL, sMainState.wSaveSubsetBttn, SENSITIVE=0
               endif
            end
         endcase
...
pro roi3d_Viz3D_CleanView, sMainState, SKIP_BUFFER=skipBuffer
...
pro roi3d_Viz3D_CleanBuffer, sMainState
   COMPILE_OPT hidden, idl2
...
pro roi3d_Viz3D_KillDraw, wDrawBase
...
pro roi3d_Viz3D_KillMain, wMainBase
...
function roi3d_slicer, hData3D, DATA_NAMES=dataNames, DETACH=detachView, $
             GROUP_LEADER=groupLead, MODAL=modal, ASPECT_RATIO=aspect_ratio

COMPILE_OPT idl2

; >>>>>>>>>>>>>>>>
; These are added by E. Ozarslan
common share1, nnn, slices, nmax, over
nmax=100
slices=make_array(6, nmax)
nnn=0
over=0
;<<<<<<<<<<<<<<<<
...
; Returns 0 because it is a function.
; E. Ozarslan
            return, 0
         endif
...
; Generic widget_base keywords for modal or nonmodal
	extra = {TITLE:'3D ROI Slice Selector (ROI3d_slicer)', $
		ROW:1,SPACE:1,XPAD:1,YPAD:1} ; Title changed. E. Ozarslan
...
;                     UVALUE='wAnimateBttn')
   ...
   if (detachBase and NOT runModal) then $
      ...
      wDrawBase = WIDGET_BASE(wMainBase, UVALUE=wMainBase, /COLUMN, $
         SPACE=1, XPAD=1, YPAD=1, EVENT_PRO='roi3d_Viz3D_SliceEvent')
...
;>>>>>>>>>>>>>>>>>>>>>
; Added by E. Ozarslan
wTextTitle=Widget_Text(wDrawBase, SCR_XSIZE=winX-12, YSize=1, $
     Value='   Slice Normal  		   Slice Index')
wText=WIDGET_TEXT(wDrawBase, SCR_XSIZE=winX-12, YSIZE=5, VALUE='', /SCROLL)
;<<<<<<<<<<<<<<<<<<<<<
...
; This is different from the previous slice selector.
; E. Ozarslan
...
;>>>>>>>>>>>>>>>>>>>>>>
;This is added by E. Ozarslan
wbase=WIDGET_BASE(wSliceBase, /COLUMN, Space=1, XPAD=1, YPAD=1, /FRAME)
wsliceDeleteBttn=WIDGET_BUTTON(wBase, Value='DELETE and START OVER', $
		 UVALUE='wSliceDeleteBttn')
wsliceDoneBttn=WIDGET_BUTTON(wbase, Value='DONE', UVALUE='wSliceDoneBttn')
;<<<<<<<<<<<<<<<<<<<<<<

   wSliceMapBase = WIDGET_BASE(wSliceBase, SPACE=1, XPAD=1, YPAD=1, /FRAME)
;>>>>>>>>>>>>>>>>>>>>>>
;/COLUMN was /ROW in the previous slice selector.
;/EXCLUSIVE was removed
; E. Ozarslan
   wSliceOrthoBase = WIDGET_BASE(wSliceMapBase, /column, SPACE=1, $
                        XPAD=1, YPAD=1);, /EXCLUSIVE)
;>>>>>>>>>>>>>>>>>>>>>>
;This is added by E. Ozarslan
wsliceOrthobase1 = widget_base(wsliceorthobase, /row, space=1, $
			xpad=1,ypad=1, /EXCLUSIVE)
;<<<<<<<<<<<<<<<<<<<<<<

;>>>>>>>>>>>>>>>>>>>>>>
;wSliceOrthoBase1 was wSliceOrthoBase in the previous slice selector.
; E. Ozarslan
   wSliceXBttn = WIDGET_BUTTON(wSliceOrthoBase1, VALUE='X', $
      UVALUE='wSliceXBttn', /NO_RELEASE)
   wSliceYBttn = WIDGET_BUTTON(wSliceOrthoBase1, VALUE='Y', $
      UVALUE='wSliceYBttn', /NO_RELEASE)
   wSliceZBttn = WIDGET_BUTTON(wSliceOrthoBase1, VALUE='Z', $
      UVALUE='wSliceZBttn', /NO_RELEASE)
;<<<<<<<<<<<<<<<<<<<<<<<

;>>>>>>>>>>>>>>>>>>>>>>
;This is added by E. Ozarslan
wSliceOrthoGrabBttn = WIDGET_BUTTON(wSliceOrthoBase, $
	VALUE='GRAB THE LAST SLICE', UVALUE='wSliceOrthoGrabBttn')

; HS 20070830
; Adding of slices manually
w_manual_input_textbox = WIDGET_TEXT(wSliceOrthoBase, $
	VALUE='', UVALUE='w_manual_input_textbox', /EDITABLE)

w_manual_input_button = WIDGET_BUTTON(wSliceOrthoBase, $
	VALUE='MANUAL GRAB SLICE', UVALUE='w_manual_input_button')


;<<<<<<<<<<<<<<<<<<<<<<

;>>>>>>>>>>>>>>>>>>>>>>
; Used to be WIDGET_CONTROL, wSliceXBttn, /SET_BUTTON.
; Changed by E. Ozarslan
   WIDGET_CONTROL, wSliceZBttn, /SET_BUTTON
...
;>>>>>>>>>>>>>>>>>>>>>
; This is added by E. Ozarslan
wSliceObliqGrabBttn = WIDGET_BUTTON(wSliceObliqBase, $
	VALUE='GRAB THE LAST SLICE', UVALUE='wSliceObliqGrabBttn')
;<<<<<<<<<<<<<<<<<<<<<<<

;>>>>>>>>>>>>>>>>>>>>>>
;The following is commented in the previous slice selector.
;E. Ozarslan
   ; Block base widgets.
...
;   WIDGET_CONTROL, wRot3Drop, SET_DROPLIST_SELECT=0
   ...
   wZFacBase = WIDGET_BASE(wViewBase, /ROW, SPACE=1, XPAD=1, YPAD=1, $
                           /FRAME)
;>>>>>>>>>>>>>
; This part is modified to use the specified aspect ratio.
; E. Ozarslan
   if n_elements(aspect_ratio) eq 0 then aspect_ratio=1.
   wZFacSlid = WIDGET_SLIDER(wZFacBase, Min=50, Max=400,  $
                 VALUE=round(sqrt(aspect_ratio)*100.)<400, $
...
;<<<<<<<<<<<<<<<<<<<<<<
   ...
   sColorState = roi3d_Viz3D_LoadColor(DIFFSHADE=20)

   ; View state.
   ; ZSCALE keyword was sent in order to initialize the image
   ; to the given aspect ratio.
   ; E.Ozarslan
   sViewState = roi3d_Viz3D_View(viewWin, ANG1=ang1, ANG2=ang2, ANG3=ang3, $
      DIR1='Z', DIR2='X', DIR3='Y', ZOOM=0.75, XMAX=szData[1]-1L, $
      YMAX=szData[2]-1L, ZMAX=szData[3]-1L, zscale=sqrt(aspect_ratio)<4.)
   ...
   ; Slice state.
   ;----------> Default value of orthoDir was 1. Changed by E. Ozarslan
   sSliceState = $
      {SSliceState, sliceWin:sliceWin, $
       ...
       wSliceZBttn:wSliceZBttn, drawMode:0, planeMode:1, orthoDir:3, $
...
;The mainstate is different in the previous slice selector.
;, wText:wText is added
; {SVizState was replaced with {SMainState
;E. Ozarslan
   ; Main state.
   sMainState = {sMainState, curData:0, $
       ...
       wStatText:wStatText, wText:wText, wSaveSubsetBttn:wSaveSubsetBttn, $
       ...
       w_manual_input_textbox:w_manual_input_textbox $
    }
...
;<<<<<<<<<<<<<<<<<
   ...
   if (runModal) then begin
      ...
      XMANAGER, 'Slicer3', wMainBase, EVENT_HANDLER='roi3d_Viz3D_Event', $
         CLEANUP='roi3d_Viz3D_KillMain'
...
; Title in the XMANAGER line is different in the previous slice selector.
; E. Ozarslan
      ; Start event processing.
      if (detachBase) then $
         XMANAGER, 'roi3d_slicer', wDrawBase, /JUST_REG, $
            EVENT_HANDLER='roi3d_Viz3D_SliceEvent', CLEANUP='roi3d_Viz3D_KillDraw'
; Title in the XMANAGER line is different in the previous slice selector.
; E. Ozarslan
      XMANAGER, 'roi3d_slicer', wMainBase, EVENT_HANDLER='roi3d_Viz3D_Event', $
         CLEANUP='roi3d_Viz3D_KillMain'
