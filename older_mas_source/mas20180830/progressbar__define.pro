...
PRO PROGRESSBAR::GetProperty, COLOR=color, FAST_LOOP=fast_loop, TEXT=text
   ...
   IF theError NE 0 THEN BEGIN
      Catch, /Cancel
      ok = ProgressBar_Error_Message(Traceback=1, /Error)
      RETURN
   ENDIF
...
PRO PROGRESSBAR::SetProperty, CANCEL=cancel, COLOR=color, FAST_LOOP=fast_loop, TEXT=text, TITLE=title
   ...
   IF N_Elements(text) NE 0 THEN Widget_Control, self.labelID, Set_Value=text
   IF N_Elements(title) NE 0 THEN Widget_Control, self.tlb, TLB_Set_Title=title
...
PRO PROGRESSBAR::Start, initialPercent
   ...
   IF self.fast THEN self.colorindex = FSC_Color(self.color, !P.Color)
   ...
   XManager, 'progressbar', self.tlb, /No_Block, Cleanup='ProgressBar_Cleanup'
...
; The text will be substituted for the message text.
   ...
   IF theError NE 0 THEN BEGIN
       ...
       ok = Progressbar_Error_Message(/Informational)
       RETURN
   ENDIF
...
;button = Widget_Button(tlb, Value='Start Loop')
